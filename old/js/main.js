...
$(document).ready(function () {
    ...
    if (
        ...
        window.pageYOffset + window.innerHeight < $(document).height()
    ) {
...
function scroll_header() {
    ...
    var opacity =
        originalHomeColorOpacity +
        (1 - originalHomeColorOpacity) * proportionScrolled
    $('#header').css(
        'background',
        'linear-gradient(to right, rgba(107, 77, 168, ' +
            String(opacity) +
            '),\
                      rgba(17, 138, 188, ' +
            String(opacity) +
            ')), url(img/higgins.jpg) no-repeat center bottom'
    )
...
function position_footer() {
    ...
    if (
        $('footer').position().top + $('footer').outerHeight() <
        window.innerHeight
    ) {
